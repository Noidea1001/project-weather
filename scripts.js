...
function renderTodayHourly(list) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        const timeLabel = (i === 0) ? '<span style="color: white;">Now</span>' : forecastDate.toLocaleTimeString([], { hour: 'numeric', hour12: true }).toLowerCase().replace(' ', '');
...
function renderSpecificDayHourly(dayItems) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        let timeLabel = (isCityToday && i === 0) ? '<span style="color: white;">Now</span>' : targetDate.toLocaleTimeString([], { hour: 'numeric', hour12: true }).toLowerCase().replace(' ', '');
